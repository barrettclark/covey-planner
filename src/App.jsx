...
import {
  fmtDate, fmtWeekday, fmtDayNum, weekDates, effectivePriority, sortedTxt,
} from "./todotxt.js";
import { startDropboxAuth } from "./dropbox.js";
import { useTaskManager } from "./useTaskManager.js";
import { C, FONT_SERIF } from "./theme.js";
import {
  PMETA, UPCOMING_META, mini, highlight,
  TaskForm, Row, Group,
  DailyView, WeeklyView, UpcomingView, SomedayView,
  HBtn, SBtn, Chip,
  HelpSection, HelpDivider, Code, hp, ul, li,
} from "./components.jsx";
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: FONT_SERIF,
      minHeight: "100vh", width: "100%", background: C.parchment, color: C.ink, overflowX: "hidden" }}>
      <style>{`
        ...
        <div className="task-col">
          ...
          <div className="task-col-inner" style={{ padding:"22px 24px 80px", flex:1 }}>
            ...
            {view === "daily" && tasks !== null && (
              <DailyView
                tasks={tasks} groups={groups} doneTasks={doneTasks}
                reschedulePrompt={reschedulePrompt} rescheduleDate={rescheduleDate}
                setRescheduleDate={setRescheduleDate} confirmReschedule={confirmReschedule}
                setReschedulePrompt={setReschedulePrompt}
                addingFor={addingFor} setAddingFor={setAddingFor}
                form={form} setForm={setForm} addTask={addTask}
                editingId={editingId} setEditingId={setEditingId}
                toggleDone={toggleDone} toggleInProgress={toggleInProgress}
                deleteTask={deleteTask} saveEdit={saveEdit}
                ...
                allProj={allProj} allCtx={allCtx}
                focusedTaskId={focusedTaskId} setFocusedTaskId={setFocusedTaskId}
                showDone={showDone} searchQuery={q} TODAY={TODAY}
              />
            )}
            ...
            {view === "weekly" && tasks !== null && (
              <WeeklyView
                tasks={tasks} weekDays={weekDays} dayTasks={dayTasks} TODAY={TODAY}
              />
            )}
            ...
            {view === "upcoming" && tasks !== null && (
              <UpcomingView
                ...
                editingId={editingId} setEditingId={setEditingId}
                saveEdit={saveEdit} deleteTask={deleteTask} makeVisible={makeVisible}
                allProj={allProj} allCtx={allCtx}
                searchQuery={q} TODAY={TODAY}
              />
            )}
            ...
            {view === "someday" && tasks !== null && (
              <SomedayView
                ...
                editingId={editingId} setEditingId={setEditingId}
                saveEdit={saveEdit} deleteTask={deleteTask} promoteToDaily={promoteToDaily}
                addingFor={addingFor} setAddingFor={setAddingFor}
                form={form} setForm={setForm} addSomedayTask={addSomedayTask}
                allProj={allProj} allCtx={allCtx}
                searchQuery={q}
              />
            )}
