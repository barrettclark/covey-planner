...
export const mini = {
  fontSize: 12, padding: "4px 8px", border: `1px solid ${C.disabledDeep}`,
  borderRadius: RADIUS.md, fontFamily: "monospace", background: C.white, color: C.ink,
};

// ─── Priority metadata ────────────────────────────────────────────────────────

export const PMETA = {
  A:   { label: "A — Vital",      accent: C.redAccent,    bg: C.redBg,     border: C.redBorder,    dot: C.redAccent    },
  B:   { label: "B — Important",  accent: C.amberAccent,  bg: C.amberBg,   border: C.amberBorder,  dot: C.amberAccent  },
  C:   { label: "C — Nice to Do", accent: C.greenAccent,  bg: C.greenBg,   border: C.greenBorder,  dot: C.greenAccent  },
  R:   { label: "R — Recurring",  accent: C.blueAccent,   bg: C.blueBg,    border: C.blueBorder,   dot: C.blueAccent   },
  "?": { label: "Unsorted",       accent: C.neutral,      bg: "#f5f4f0",   border: "#d8d5ce",      dot: C.neutral      },
};

// Upcoming tab meta (for edit form theming)
export const UPCOMING_META = { accent: C.purpleAccent, bg: C.purpleBg, border: C.purpleBorder, dot: C.purpleAccent };
  ...
  return (
    <div style={{
      ...
      <input
        ...
        autoFocus
        style={{
          width: "100%", border: `1px solid ${meta?.border || "#ddd"}`, borderRadius: 4,
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          ...
          <select value={f.priority || "C"} onChange={e => setF(p => ({ ...p, priority: e.target.value }))}
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
          {allProjects.map(p => (
            <button key={p} onClick={() => toggleTag("project", p)}
              style={{
                fontSize: 11, fontFamily: "monospace", padding: "2px 8px", borderRadius: 12, border: "1px solid", cursor: "pointer",
      ...
      {/* In progress */}
      <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: isEditMode ? 10 : 12 }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <SBtn onClick={handleSubmit} color={meta?.accent || "#888"}>{submitLabel}</SBtn>
...
export function Row({
  task, idx, meta, groupPriority, editingId, setEditingId,
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <div
          style={{ display: "flex", alignItems: "flex-start", padding: "9px 12px", gap: 8, transition: "background 0.1s" }}
        ...
        >
          ...
          <div style={{
            minWidth: 24, textAlign: "center", fontSize: 11, fontWeight: "bold",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0, alignItems: "center" }}>
            <button onClick={onToggleInProgress} title={task.inProgress ? "Clear in-progress" : "Mark in-progress"}
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: task.inProgress ? "#b07010" : "#ddd", padding: "2px 4px" }}>▶</button>
...
export function Group({
  priority, meta, tasks, addingFor, setAddingFor, form, setForm, onAdd,
  ...
  onTouchDragStart, onTouchDragMove, onTouchDragEnd,
  allProjects, allContexts, focusedTaskId, setFocusedTaskId, searchQuery, TODAY,
}) {
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            width: 22, height: 22, borderRadius: "50%", background: meta.accent, color: "#fff",
        ...
        <button
          ...
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: meta.accent, lineHeight: 1, padding: "0 6px", fontFamily: "inherit" }}
        >+</button>
      </div>
      ...
      <div style={{ background: meta.bg, border: `1px solid ${meta.border}`, borderRadius: 6, overflow: "hidden" }}>
        {tasks.length === 0
          ...
          : tasks.map((task, idx) => (
            <Row
              key={task.id} task={task} idx={idx} meta={meta} groupPriority={priority}
...
export function HBtn({ children, onClick }) {
  return (
    <button onClick={onClick} style={{
      background: C.inkMid, color: C.sand, border: "none",
      borderRadius: RADIUS.md, padding: "5px 12px", cursor: "pointer", fontSize: 11,
...
export function SBtn({ children, onClick, color }) {
  return (
    <button onClick={onClick} style={{
      background: color || C.neutral, color: C.white, border: "none",
      borderRadius: RADIUS.md, padding: "4px 10px", cursor: "pointer", fontSize: 12, fontFamily: "inherit",
    }}>{children}</button>
  );
}
...
export function HelpSection({ title, children }) {
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{
        fontSize: 13, fontWeight: "bold", letterSpacing: "0.05em", textTransform: "uppercase",
        color: C.ink, marginBottom: 10, paddingBottom: 4, borderBottom: `2px solid ${C.e8e0d0 || "#e8e0d0"}`,
      }}>
        {title}
      </div>
      {children}
...
export function Code({ children }) {
  return (
    <code style={{
      background: "#e8e0d0", borderRadius: RADIUS.sm, padding: "1px 5px",
      fontSize: 11, fontFamily: "monospace", color: C.inkLight,
    }}>{children}</code>
  );
}
...
export function DailyView({
  tasks, groups, doneTasks,
  reschedulePrompt, rescheduleDate, setRescheduleDate, confirmReschedule, setReschedulePrompt,
  addingFor, setAddingFor, form, setForm, addTask,
  editingId, setEditingId, toggleDone, toggleInProgress, deleteTask, saveEdit,
  dragId, dragOverId, dragOverGroup, setDragOverGroup, setDragId, setDragOverId, onDrop, onDropGroup,
  handleTouchDragStart, handleTouchDragMove, handleTouchDragEnd,
  allProj, allCtx, focusedTaskId, setFocusedTaskId,
  showDone, searchQuery, TODAY,
}) {
  return (
    <>
      {reschedulePrompt && (
      ...
      {["A", "B", "C", "?"].map(p => (
        <Group
          key={p} priority={p} meta={PMETA[p]} tasks={groups[p] || []}
...
export function UpcomingView({
  upcomingTasks, editingId, setEditingId, saveEdit, deleteTask, makeVisible,
  allProj, allCtx, searchQuery, TODAY,
}) {
  return (
    <>
                      ...
                      return (
                        <div key={task.id} style={{ borderTop: idx > 0 ? "1px solid #c9b8e8" : "none" }}>
                          {isEdit ? (
                            ...
                            <div
                              style={{ display: "flex", alignItems: "flex-start", padding: "10px 14px", gap: 10, transition: "background 0.1s" }}
                              onMouseEnter={e => e.currentTarget.style.background = "#ede8f5"}
                              onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                            >
                              <div style={{
                                width: 22, height: 22, borderRadius: "50%", background: m.accent,
                                color: "#fff", display: "flex", alignItems: "center", justifyContent: "center",
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div
                                  style={{ fontSize: 14, color: "#1e1810", lineHeight: 1.4, cursor: "pointer" }}
                                  onClick={() => setEditingId(task.id)}
                                  title="Click to edit"
                                >
                                  {searchQuery ? highlight(task.cleanText, searchQuery) : task.cleanText}
                              ...
                              <div style={{ display: "flex", gap: 4, flexShrink: 0, alignItems: "center" }}>
                                ...
                                <button onClick={() => setEditingId(task.id)} title="Edit"
                                  style={{ background: "none", border: "none", cursor: "pointer", fontSize: 13, color: "#c9b8e8", padding: "2px 4px" }}>✎</button>
...
export function SomedayView({
  somedayTasks, editingId, setEditingId, saveEdit, deleteTask, promoteToDaily,
  addingFor, setAddingFor, form, setForm, addSomedayTask,
  allProj, allCtx, searchQuery,
}) {
  return (
    <>
      <div style={{ marginBottom: 18 }}>
                    ...
                    <div style={{ borderTop: "1px solid #e8d8b0", display: "flex", alignItems: "center", padding: "6px 10px", gap: 6 }}>
                      ...
                      <button onClick={() => setEditingId(task.id)}
                        style={{ background: "none", border: "1px solid #ddc898", borderRadius: 4,
                          padding: "5px 8px", cursor: "pointer", fontSize: 11, color: "#8a7060", fontFamily: "inherit" }}>
                        Edit
                      </button>
