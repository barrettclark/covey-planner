...
describe("Row", () => {
  ...
  it("calls onToggle when the checkbox is clicked", () => {
    ...
    renderRow({}, { onToggle });
    // The checkbox is the small square div next to the drag handle in the
    // non-editing layout — locate the row, then the clickable square.
    ...
    const checkboxDiv = row?.querySelector(
      '[style*="cursor: pointer"][style*="width: 16px"], [style*="cursor:pointer"]',
    );
  ...
  it("renders the group label", () => {
    render(
      <Group
        ...
        setEditingId={noop}
        onToggle={noop}
        onToggleInProgress={noop}
        onDelete={noop}
        onSaveEdit={noop}
        dragId={null}
        dragOverId={null}
        dragOverGroup={null}
        setDragOverGroup={noop}
        setDragId={noop}
        setDragOverId={noop}
        onDrop={noop}
        onDropGroup={noop}
