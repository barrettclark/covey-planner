...
export function useTaskManager() {
  ...
  const nextId = useRef(Math.max(0, ...(hasDropbox ? [] : SAMPLE.map((_, i) => i + 1))) + 1);
  ...
  async function openFile() {
    try {
      const [handle] = await window.showOpenFilePicker({
        types: [
          {
            description: "todo.txt / todo.todotxt",
            accept: { "text/plain": [".txt", ".todotxt"] },
          },
        ],
      });
  ...
  function toggleDone(id) {
    setTasks(prev => {
      ...
      if (!task.done && task.recurrence) {
        ...
        return prev
          .map(t => (t.id === id ? { ...t, done: true, completedDate: TODAY } : t))
          .concat(next);
      }
      return prev.map(t =>
        t.id === id ? { ...t, done: !t.done, completedDate: !t.done ? TODAY : null } : t,
      );
    });
  }
  ...
  function addTask(priority) {
    ...
    const basePriority =
      form.priority && form.priority !== "?" ? form.priority : priority === "?" ? "C" : priority;
  ...
  function promoteToDaily(id) {
    setTasks(prev =>
      prev.map(t => (t.id === id ? { ...t, dueDate: TODAY, priority: t.priority || "C" } : t)),
    );
    flash("✓ Moved to today's list");
  }

  function makeVisible(id) {
    setTasks(prev => prev.map(t => (t.id === id ? { ...t, thresholdDate: null } : t)));
  ...
  function handleTouchDragMove(e) {
    ...
    while (node && node !== document.body) {
      if (node.dataset?.taskid) {
        foundTaskId = parseInt(node.dataset.taskid);
        break;
      }
      if (node.dataset?.group) {
        foundGroup = node.dataset.group;
        break;
      }
  ...
  function applyReprioritize(dragged, newPriority, insertBeforeId) {
    if (dragged.priority === "R") {
      ...
      setTasks(prev => {
        let arr = prev.map(t =>
          t.id === dragged.id ? { ...t, priority: newPriority === "?" ? null : newPriority } : t,
        );
        if (insertBeforeId) {
  ...
  function onDrop(targetId) {
    if (!tasks || !dragId || dragId === targetId) {
      setDragId(null);
      setDragOverId(null);
      return;
    }
  ...
  function startPlanningMode() {
    const overdue = (tasks || []).filter(
      t => !t.done && t.dueDate && t.dueDate < TODAY && t.priority !== "R",
    );
  ...
  const allCtx = useMemo(
    () => [...new Set((tasks || []).flatMap(t => t.contexts))].sort(),
    [tasks],
  );
  const allProj = useMemo(
    () => [...new Set((tasks || []).flatMap(t => t.projects))].sort(),
    [tasks],
  );
  ...
  function getFilteredViews({ searchQuery, showDone, filterCtx, filterProj }) {
    ...
    const upcomingTasks = (tasks || [])
      .filter(t => !t.done && t.thresholdDate && t.thresholdDate > TODAY && matchesSearch(t, q))
      .sort((a, b) => {
        if (a.thresholdDate !== b.thresholdDate)
          return a.thresholdDate.localeCompare(b.thresholdDate);
    ...
    visibleActive.forEach(t => {
      const ep = effectivePriority(t, TODAY);
      const k =
        ep && ["A", "B", "C"].includes(ep)
          ? ep
          : t.priority && ["A", "B", "C"].includes(t.priority)
            ? t.priority
            : "?";
      groups[k].push(t);
    });
    Object.keys(groups).forEach(k => {
      groups[k].sort((a, b) => {
        ...
        const ka = dueSortKey(a, TODAY),
          kb = dueSortKey(b, TODAY);
        if (ka !== kb) return ka - kb;
        if (ka === 2) return a.dueDate.localeCompare(b.dueDate);
        const as = a.seq ?? 99999,
          bs = b.seq ?? 99999;
        return as - bs;
      });
    });
